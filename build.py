...
def render_navigation(page_name: str) -> str:
    ...
    rendered = NAV_ACTIVE_RE.sub(substitute, nav)
    rendered = re.sub(
        r'class="([^"]*\bnav-active\b[^"]*)"(?=\s+href=)',
        r'class="\1" aria-current="page"',
        rendered,
    )
    rendered = re.sub(
        r'class="([^"]*\bnav-active\b[^"]*)"(?=\s+aria-expanded=)',
        r'class="\1" aria-current="true"',
        rendered,
    )

    lower_name = page_name.lower()
    if lower_name.startswith("music"):
        current_value = "page" if lower_name == "music.html" else "true"
        rendered = rendered.replace(
            'href="music.html"',
            f'class="nav-sub-active" aria-current="{current_value}" href="music.html"',
            1,
        )
    elif lower_name.startswith("albums"):
        current_value = "page" if lower_name == "albums.html" else "true"
        rendered = rendered.replace(
            'href="albums.html"',
            f'class="nav-sub-active" aria-current="{current_value}" href="albums.html"',
            1,
        )
...
def render_album_years(page_name: str) -> str:
    ...
    def substitute(match: re.Match[str]) -> str:
        year = match.group(1)
        if year == active_year:
            return f'href="albums.html#{year}" class="nav-active" aria-current="page"'
...
def render_music_years(page_name: str) -> str:
    ...
    if page_name == "music-old.html":
        ...
        return rendered.replace('href="#OlderTracks"', 'href="music-old.html" class="nav-active" aria-current="page"')
    ...
    def substitute(match: re.Match[str]) -> str:
        year = match.group(1)
        if year == active_year:
            return f'href="music.html#{year}" class="nav-active" aria-current="page"'
